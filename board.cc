...
void Board::addPiece(char piece, std::string pos) { // may be call the addPiece in the piece
    // need to change the sate of the all possible next moved of the moved piece
    // into check (white, black, or both) in order to print out check message as well as check if the king is checkmate
    std::vector intPos = intPos(pos);
    int r = intPos[1];
    int c= intPos[0];
    ...
    if (theBoard[r][c] != ' ' && theBoard[r][c] != '_') {
        ...
        else theBoard[r][c].removePiece(piece, pos);
    }
    theBoard[r][c].addPiece(piece, pos);
}

void Board::removePiece(std::string pos) { // may be call the removePiece in the piece
    // need to delete the piece of all possible next move without check of the following color
    // as well as remove the piece of the position 
    // next move is generated by the piece type obatined by getPiece (or something else) of the piece
    std::vector intPos = intPos(pos);
    int r = intPos[1];
    int c= intPos[0];

    theBoard[r][c].removePiece(piece, pos)
}
